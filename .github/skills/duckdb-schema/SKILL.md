...
| `revise_history` | 495 | 可转债转股价下修历史（集思录） | `(bond_code, meeting_date)` |
...
- **⚠️ 下修条件全部为默认值 85/15/30**（无法从API获取实际值，绝大多数转债确实是此条件）
...
**数据分布**:
- 覆盖2017-2026年，共495条下修记录，涉及329只转债
- 2024年为下修大年（206次），2023年62次，2025年78次
- 下修到底率：2019-2020年约90-100%，2023-2025年降至45-56%
- 下修最多的债：蓝帆转债(8次)、维尔转债(6次)、汇车退债(6次)

**更新方式**: `python db/fetch_revise_history.py`（集思录数据源，全量刷新）
